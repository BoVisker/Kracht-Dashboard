...
What's genuinely still missing, as honest stubs rather than silently-broken features: a training-plan editor (defining a schedule in advance, distinct from the session classification that already works), and units/privacy settings (kg/lbs, km/mi, data export/delete).
